...
const GuideDashboard = () => {
    ...
    useEffect(() => {
        ...
        if (storedUserId) {
            ...
            fetchUserIdByUsername(storedUsername);
        }
    }, []);
    ...
    const fetchUserIdByUsername = async (username) => {
        ...
        try {
            const response = await axios.get(`http://localhost:8080/users/username/${encodeURIComponent(username)}`);
            console.log('Fetched user ID:', response.data);
            setUserId(response.data.userId); // Make sure response.data contains userId
            fetchAssignedTours(response.data.userId);
        } catch (error) {
            console.error("Error fetching user ID by username:", error.response ? error.response.data : error.message);
        }
    };
                            ...
                            <ul className="list-group">
                                {assignedTours.map((tour) => (
                                    <li key={tour.bookingId} className="list-group-item">
                                        <strong>
                                            {tour.tourPackage && tour.tourPackage.pname ? tour.tourPackage.pname : 'Package'}
                                        </strong><br />
